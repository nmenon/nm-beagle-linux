			...
			struct ieee80211_sta *sta, u8 *hlid, u8 is_connected)
{
	...
	if(sta->ht_cap.ht_supported)
	{
		...
		cmd->ht_capabilities|= CC33XX_HT_CAP_HT_OPERATION;
...
int cmd_smart_config_start(struct cc33xx *wl, u32 group_bitmap)
{
...
int cmd_smart_config_stop(struct cc33xx *wl)
{
	struct cc33xx_cmd_header *cmd;
	int ret = 0;
...
int cmd_smart_config_set_group_key(struct cc33xx *wl, u16 group_id,
					  u8 key_len, u8 *key)
{
