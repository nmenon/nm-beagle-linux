...
#include "scan.h"
#include "sysfs.h"
//#include "../cc33xx/scan.h"
#include "../net/mac80211/ieee80211_i.h"


#define CC33XX_WAKEUP_TIMEOUT 500
#define CC33XX_FW_RX_PACKET_RAM 9000
...
#ifdef TODO_REG_DOMAIN //need a regulatory domain support
...
const static struct ieee80211_sta_vht_cap sta_vht_cap_5ghz = {
	.vht_supported = true,
...
static void cc33xx_flush_deferred_work(struct cc33xx *wl)
{
	struct sk_buff *skb;
...
static int wlcore_irq_locked(struct cc33xx *wl)
{
	...
	size_t maximum_rx_packet_ram_size = CC33XX_FW_RX_PACKET_RAM;
	...
	if (rx_byte_count != 0)
	{
		...
		if (wl->max_transaction_len)//Will Enter here incase we are in SPI
		{
			read_data_len = 
				min(read_data_len, wl->max_transaction_len);		
		}
		else //Will Enter here if we are in SDIO
		{
			/*double the read_data_length, to pay for the overhead/delay payed from the raw_read which gives us an outdated core status*/
			read_data_len*=2;
			if (read_data_len > maximum_rx_packet_ram_size)
			{
				read_data_len = 
					min(read_data_len,maximum_rx_packet_ram_size);
			}
		}
...
static void cc33xx_read_fwlog_panic(struct cc33xx *wl)
{
	u32 end_of_log = 0;
	int error;
...
{
	struct cc33xx *wl =
		container_of(work, struct cc33xx, recovery_work);
	struct cc33xx_vif *wlvif;
	struct ieee80211_vif *vif;
	int error;
...
static void cc33xx_turn_off(struct cc33xx *wl)
{
	...
	wl->rx_counter = 0;
	wl->power_level = CC33XX_DEFAULT_POWER_LEVEL;
				   ...
				   struct ieee80211_vif *vif)
{
	...
	cc33xx_debug(DEBUG_MAC80211, "mac80211 name1=%s, name2=%s, name3=%s",
		     sdata->name, sdata->dev->name, sdata->wdev.netdev->name);

	if (vif->type == NL80211_IFTYPE_STATION)
	{
		//check if sta interface is primary or secondary
		//i.e 0 - primary (default)
		if( strcmp("wlan0", sdata->wdev.netdev->name) != 0)
		{
			sta_role_idx = 1;
		}
	}
	wl->sta_role_idx = sta_role_idx;
	cc33xx_debug(DEBUG_MAC80211, "mac80211 sta_role_idx=%d", sta_role_idx);
			     ...
			     struct ieee80211_vif *vif)
{
	...
	ret = cc33xx_acx_tsf_info(wl, wlvif, &mactime);
	if (ret < 0)
		goto out_sleep;
...
static int cc33xx_get_hw_info(struct cc33xx *wl)
{
	int ret = 0;
...
static int cc33xx_init_ieee80211(struct cc33xx *wl)
{
	...
	wl->hw->extra_tx_headroom = sizeof(struct cc33xx_tx_hw_descr);
	...
	*/
	...
	for (i = 0; i < ARRAY_SIZE(cc33xx_channels_5ghz); i++) {
		...
		cc33xx_band_5ghz.channels[i].max_power = WLCORE_MAX_TXPWR;
...
static int __maybe_unused wlcore_runtime_resume(struct device *dev)
{
	...
	if (!pending) {
		ret = wait_for_completion_timeout(&compl,
			msecs_to_jiffies(CC33XX_WAKEUP_TIMEOUT));
		if (ret == 0) {
...
static int cc33xx_conf_init(struct cc33xx *wl, struct device *dev)
{
	struct platform_device *pdev = wl->pdev;
...
static int cc33xx_setup(struct cc33xx *wl)
{
	...
	BUILD_BUG_ON(CC33XX_MAX_LINKS > WLCORE_MAX_LINKS);
...
static int cc33xx_probe(struct platform_device *pdev)
{
	...
	if (ret)
		goto out_free;
	cc33xx_info("[wifi CC33xx sdio] platform device probe done");
	return ret;
